...
@if (!$posts->isEmpty())
    @foreach ($posts as $post)
        <div class="sm:grid grid-cols-2 gap-10 border-dashed border-b-2 border-gray-300">
            ...
            <div class="py-2 px-2">
                <h2 class="text-gray-700 font-bold text-2xl leading-tight pb-4">
                    {{ $post->title }}
                </h2>
    ...
    @endforeach
@else
    {{-- {{ eloquent라서 null이 나와도 empty()메소드로는 확인이 안됨, 그래서 isEmpty()메소드 사용 }} --}}
    <div class="py-10">
        <h1 class="text-gray-600 italic font-bold text-center"> 해당 카테고리가 없습니다. </h1>
    </div>
@endif
